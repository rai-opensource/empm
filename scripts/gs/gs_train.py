...
if __name__ == "__main__":
    ...
    if args.scene:
        experiments = [{"case_name": args.scene}]
    else:
    ...
    for exp in experiments:
        scene = exp["case_name"]
        print(f"\n{'='*60}\nProcessing: {scene}\n{'='*60}")
        args.source_path = f"{input_dir}/{scene}"
        args.model_path = f"{output_dir}/{scene}/{exp_name}"
        args.iterations = 500
        args.lambda_depth = 0.001
        args.lambda_normal = 0.0
        args.lambda_anisotropic = 0.0
        args.lambda_seg = 1.0
        args.use_masks = True
        args.isotropic = True
        args.gs_init_opt = "hybrid"
