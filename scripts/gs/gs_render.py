...
if __name__ == "__main__":
    ...
    for exp in experiments:
        ...
        sys.argv = [sys.argv[0],
                    "-s", f"{input_dir}/{scene}",
                    "-m", f"{output_dir}/{scene}/{exp_name}"]
        args = get_combined_args(parser)
        safe_state(args.quiet)
        render_sets(model.extract(args), args.iteration, pipeline.extract(args),
                    args.skip_train, args.skip_test, SPARSE_ADAM_AVAILABLE, args.remove_gaussians)

        iteration = args.iteration if args.iteration > 0 else 500
        render_dir = f"{output_dir}/{scene}/{exp_name}/test/ours_{iteration}/renders"
        if os.path.isdir(render_dir):
            images_to_video(render_dir, f"{video_dir}/{scene}/{exp_name}.mp4")

        print(f"Rendering complete for {scene}.")
