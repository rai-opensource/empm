...
class MPMWARPDiff(object):
    ...
    def add_geometry_collider(self, geo_args: dict):
        collider_param = Dirichlet_collider()
        # Default parameters
        geo_type = -1
        surface = "sticky"
        collider_param.active = True
        collider_param.start_time = 0.0
        collider_param.end_time = 999.0
        collider_param.friction = 0.0
        if "surface" in geo_args:
            surface = geo_args["surface"]
        if "friction" in geo_args:
            collider_param.friction = geo_args["friction"]
        if "start_time" in geo_args:
            collider_param.start_time = geo_args["start_time"]
        if "end_time" in geo_args:
            collider_param.end_time = geo_args["end_time"]
        if "velocity" in geo_args:
            v = geo_args["velocity"]
            collider_param.velocity = wp.vec3(v[0], v[1], v[2])

        # Set surface type
        if surface == "sticky" and collider_param.friction != 0:
            raise ValueError("friction must be 0 on sticky surfaces.")
        if surface == "sticky":
            collider_param.surface_type = 0
        elif surface == "slip":
        ...
        if "shape" not in geo_args:
            raise ValueError("geo_args must contain 'shape' key.")

        # Geometry-specific parameters
        if geo_args["shape"] == "cube":
            # expects geo_args: {"center": [x,y,z], "size": [sx,sy,sz]}
            geo_type = 1  # warp kernel does not support string compare
            center = geo_args["center"]
            size = geo_args["size"]
            collider_param.point = wp.vec3(center[0], center[1], center[2])
            collider_param.size = wp.vec3(size[0], size[1], size[2])
        elif geo_args["shape"] == "sphere":
            # expects geo_args: {"center": [x,y,z], "radius": r}
            geo_type = 2
            center = geo_args["center"]
            radius = geo_args["radius"]
            collider_param.point = wp.vec3(center[0], center[1], center[2])
            collider_param.radius = radius
        elif geo_args["shape"] == "cylinder":
            # expects geo_args: {"center": [x,y,z], "axis": [nx,ny,nz], "radius": r, "half_height": h}
            geo_type = 3
            center = geo_args["center"]
            axis = geo_args["axis"]
            radius = geo_args["radius"]
            half_height = geo_args["half_height"]
            # Normalize axis
            axis_scale = 1.0 / wp.sqrt(float(sum(x**2 for x in axis)))
            axis = [axis_scale * x for x in axis]
            collider_param.point = wp.vec3(center[0], center[1], center[2])
            collider_param.normal = wp.vec3(axis[0], axis[1], axis[2])
            collider_param.radius = radius
            collider_param.half_height = half_height
        elif geo_args["shape"] == "halfspace" or geo_args["shape"] == "plane":
            # expects geo_args: {"point": [x,y,z], "normal": [nx,ny,nz]}
            geo_type = 4
            point = geo_args["point"]
            normal = geo_args["normal"]
            normal_scale = 1.0 / wp.sqrt(float(sum(x**2 for x in normal)))
            normal = [normal_scale * x for x in normal]
            collider_param.point = wp.vec3(point[0], point[1], point[2])
            collider_param.normal = wp.vec3(normal[0], normal[1], normal[2])
        else:
            raise ValueError(f"Unknown geometry type: {geo_args['shape']}")
        ...
        def collide(
            ...
            # [TODO]: support moving geometry by updating param.point based on time
            # [TODO]: support different boundary types
            grid_x, grid_y, grid_z = wp.tid()
            if time >= param.start_time and time < param.end_time and param.active:
                pos = wp.vec3(float(grid_x) * model.dx, float(grid_y) * model.dx, float(grid_z) * model.dx)
                if geo_type == 1: # cube
                    offset = pos - param.point
                    if (
                        wp.abs(offset[0]) < param.size[0] and
                        wp.abs(offset[1]) < param.size[1] and
                        wp.abs(offset[2]) < param.size[2]
                    ):
                    ...
                    offset = pos - param.point
                    axis = param.normal
                    proj = wp.dot(offset, axis)
                    radial = offset - proj * axis
                    if wp.abs(proj) < param.half_height and wp.length(radial) < param.radius:
                        state.grid_v_out[grid_x, grid_y, grid_z] = wp.vec3(0.0, 0.0, 0.0)
                elif geo_type == 4: # halfspace
                    offset = pos - param.point
                    n = param.normal
                    dotproduct = wp.dot(offset, n)
                    if dotproduct < 0.0:
                        if param.surface_type == 0:
                            ...
                            # Example: restrict z range for 'cut' type
                            if (
                                float(grid_z) * model.dx < 0.4
                                or float(grid_z) * model.dx > 0.53
                            ):
    ...
    def set_grid_velocity_on_geometry(self, geo_args: dict):
        """
        Set velocity on grid nodes inside the specified geometry.
        geo_args: dict, same format as add_geometry_collider
        velocity: [vx, vy, vz]
        """
        collider_param = Dirichlet_collider()
        # Default parameters
        ...
        collider_param.start_time = 0.0
        collider_param.end_time = 999.0
        collider_param.velocity = wp.vec3(0.0, 0.0, 0.0)
        collider_param.angular_velocity = 0.0
        collider_param.rot_axis = wp.vec3(1.0, 0.0, 0.0)
        collider_param.reset = 0        
        # Set optional parameters
        if "velocity" in geo_args:
            v = geo_args["velocity"]
            collider_param.velocity = wp.vec3(v[0], v[1], v[2])
        if "angular_velocity" in geo_args:
            collider_param.angular_velocity = geo_args["angular_velocity"]
        if "rot_axis" in geo_args:
            r = geo_args["rot_axis"]
            collider_param.rot_axis = wp.vec3(r[0], r[1], r[2])
        if "start_time" in geo_args:
            collider_param.start_time = geo_args["start_time"]
        if "end_time" in geo_args:
            collider_param.end_time = geo_args["end_time"]
        if "reset" in geo_args:
            collider_param.reset = geo_args["reset"]
        ...
        def collide(
            ...
            if time >= param.start_time and time < param.end_time and param.active:
                ...
                offset = pos - param.point
                v = param.velocity
                v = v + param.angular_velocity * wp.cross(param.rot_axis, offset)
                if geo_type == 1: # cube
                    if (
                        wp.abs(offset[0]) < param.size[0] and
                        wp.abs(offset[1]) < param.size[1] and
                    ...
                    if wp.abs(proj) < param.half_height and wp.length(radial) < param.radius:
                        state.grid_v_out[grid_x, grid_y, grid_z] = v
                elif geo_type == 4: # halfspace/plane
                    n = param.normal
                    dotproduct = wp.dot(offset, n)
                    if dotproduct < 0.0:
                        state.grid_v_out[grid_x, grid_y, grid_z] = v
    ...
    def set_particle_mover(
        ...
        """
        General function to enforce particle velocity by mask or geometry.
        velocity: [vx, vy, vz] (linear velocity)
        angular_velocity: scalar (rad/s)
        center: [cx, cy, cz] (rotation center)
        axis: [ax, ay, az] (rotation axis)
        selection_mask: torch.Tensor (n_particles,) or None
        geo_args: dict (same as add_geometry_collider) or None
        """
        param = ParticleVelocityModifier()
        param.start_time = start_time
        param.end_time = end_time

        # # Set mask
        if selection_mask is not None:
            param.mask = wp.from_torch(torch.tensor(selection_mask, dtype=torch.int32))
        else:
            param.mask = wp.ones(shape=self.n_particles, dtype=int, device=device)

        # Set velocity and rotation info
        if velocity is not None:
            param.velocity = wp.vec3(velocity[0], velocity[1], velocity[2])
        if angular_velocity is not None and center is not None and axis is not None:
            param.rotation_axis = wp.vec3(axis[0], axis[1], axis[2])
            param.rotation_center = wp.vec3(center[0], center[1], center[2])
            param.angular_velocity = angular_velocity

        self.particle_velocity_modifier_params.append(param)
        ...
        def modify_particle_v_before_p2g(
            ...
            if (time >= param.start_time and time < param.end_time):
                if param.mask[p] == 1:
                    # Linear velocity
                    v = state.particle_v[p] + param.velocity
                    # Angular velocity
                    offset = state.particle_x[p] - param.rotation_center
                    v = v + param.angular_velocity * wp.cross(param.rotation_axis, offset)
