'''
    Modified from https://github.com/a1600012888/PhysDreamer/blob/main/projects/uncleaned_train/exp_motion/train/interface.py
'''
...
from warp_utils import from_torch_safe, MyTape, CondTape
from mpm_solver_diff import MPMWARPDiff
from mpm_utils import compute_position_l2_loss, aggregate_grad, compute_posloss_with_grad
from mpm_data_structure import MPMStateStruct, MPMModelStruct, get_float_array_product
from mpm_utils import (compute_Closs_with_grad, compute_Floss_with_grad, 
                                                compute_posloss_with_grad, compute_veloloss_with_grad)

class MPMDifferentiableSimulationTrail(autograd.Function):
    ...
    def forward(
        ...
        ##### [TODO]: reset sim params
        # mpm_solver.collider_params[1].point = 
        # print(E.ndim, E_inp)
        # print(mpm_model.gravitational_accelaration)
        # print(mpm_solver.collider_params[1])
        # print(torch.mean(init_pos, dim=0), torch.mean(init_velocity, dim=0))
        ...
        with cond_tape:
            ...
            for substep_local in range(num_substeps):
                if mpm_solver.step_callback is not None:
                    mpm_solver.step_callback(substep_local)
                next_state = prev_state.partial_clone(requires_grad=requires_grad)
                mpm_solver.p2g2p_differentiable(mpm_model, prev_state, next_state, substep_size, device=device)
        ...
        # Convert all step positions to torch tensors, stacked [T, N, 3]
        all_positions = torch.stack([
            wp.to_torch(state.particle_x).detach().clone()
            for state in next_state_list[::substep]], dim=0)

        return all_positions  # shape: [num_substeps, num_particles, 3]
    

    @staticmethod
    def backward(ctx, out_pos_grad: Float[Tensor, "t n 3"]):
        ...
        # Reduce gradients over time dimension, e.g., sum or mean
        out_pos_grad = out_pos_grad.sum(dim=0)  # shape: [num_particles, 3]
