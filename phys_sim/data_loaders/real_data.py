...
class RealData:
    def __init__(self, visualize=False, save_gt=True):
        ...
        if DATASET == "pet": # offset for pet
            offset = torch.tensor([0, -0.015, 0], dtype=torch.float32, device=cfg.device)
            self.object_points += offset
            self.controller_points += offset
            self.structure_points += offset
